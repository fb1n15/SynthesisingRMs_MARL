...
class ButtonsEnv:
    ...
    def get_mdp_label(self, s, s_next, u):
        ...
        prev_row, prev_col = self.get_state_description(s)

        l = []

        thresh = 0.3  # why is this 0.3?

        if self.agent_id == 1:
            if u == 0 or (u == 1 and self.nonmarkovian):
                if (row, col) == self.env_settings['yellow_button']:
                    if u == 0 and self.nonmarkovian:
                        l.append('by1')
                    else:
                        l.append('by')
            ...
            if not self.strategy_rm:
                if u == 2 and (row, col) == self.env_settings['red_button']:  # agent 2 is on the red button
                    l.append('a2br')
                if u == 3:
                    if not ((row, col) == self.env_settings['red_button']):  # agent 2 leaves the red button
                        l.append('a2lr')
                    elif np.random.random() <= thresh:  # simulate the red button press
                        l.append('br')
                if u == 4 and (row, col) == self.env_settings['purple_button']:  # agent 2 is on the purple button
                    l.append('a2bp')
                if u == 5:
                    if not ((row, col) == self.env_settings['purple_button']):  # agent 2 leaves the purple button
                        l.append('a2lp')
                ...
                if u == 2 and (row, col) == self.env_settings['red_button'] \
                    ...
                    l.append('br')
                if u == 3 and (row, col) == self.env_settings['purple_button'] \
    ...
    def unlock_wall(self, color):
        """
        Unlocks the color wall.
        Parameters
        ----------
        color : str
    ...
    def get_strategy_label(self, s, s_next, option=None):
        """
        Return the label of the next environment state.
        """
        row, col = self.get_state_description(s)
        next_row, next_col = self.get_state_description(s_next)
    ...
    def get_strategy_next_state(self, s, a):
        """
        Get the next state in the environment given action a is taken from state s.
        Update the last action that was truly taken due to MDP slip.
