...
class NewMultiAgentButtonsEnv:
    ...
    def __init__(self, rm_file, num_agents, env_settings, strategy_rm=False, nonmarkovian=False, verbose=False):
        ...
        env_settings : dict
            ...
            e.g., {'Nr': 10, 'Nc': 10, 'initial_states': [0, 5, 8], 'walls': [(0, 3), (1, 3), (2, 3), (3, 3), (4, 3), (5, 3), (6, 3), (7, 3), (8, 3), (9, 3)]}
        """
        ...
        if agent_id == 0:
            ...
            if self.strategy_rm:
                ...
                if (self.u == 0) or (self.u == 1) or (self.u == 2) or (self.u == 3) or (self.u == 4) or (self.u == 5) \
                        or (self.u == 6 and self.nonmarkovian):
                    if (row, col) in self.red_tiles:
                        s_next = s
        if agent_id == 1:
            if self.u == 0:
    ...
    def get_next_agent_option(self, agent_id, rm_state):
        ...
        """
        available_options = ['w' + str(agent_id + 1)]

        # If the option of the agent should be to wait in the current RM state but there is a future state in which she
